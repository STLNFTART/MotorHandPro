...
static constexpr double N_CUT_CONST=149.9992313999;
...
static constexpr double F_PRIME_D_CONST=0.000129931830;
...
inline double planckCutValue(double Xc){ return 150.0*(1.0 - planckTail(Xc)/PLANCK_I3); }
...
inline double kernelIterate(double mu,double x0,double tol=TOL_FP,int maxIt=MAX_IT){
  auto kp=kernelParamsFor(mu); double x=x0;
...
inline double kernelSlopeAt(double mu,double x){
  auto kp=kernelParamsFor(mu); return kp.c*mu*exp(-mu*x);
}

struct Results{ double D,I3,mu,S,Xc,deltaXc,Ncut,c,FprimeD,xFixed; };

inline Results computeAll(double mu=KERNEL_MU){
  Results r{};
  r.D=PLANCK_D; r.I3=PLANCK_I3; r.mu=mu; r.S=r.D/r.I3;
  r.Xc=solveCutoffXc();
  r.deltaXc=planckTail(r.Xc)/r.I3;
