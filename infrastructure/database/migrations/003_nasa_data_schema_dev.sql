-- NASA Data Schema Migration (Non-TimescaleDB version for development)
...
-- Create hypertable for time-series data (TimescaleDB) - Skip if extension not available
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'timescaledb') THEN
        ...
        RAISE NOTICE 'Created hypertable for comet_observations';
    ELSE
        RAISE NOTICE 'TimescaleDB not available - using regular table for comet_observations';
    END IF;
END $$;
...
-- Create hypertable for processed states - Skip if extension not available
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'timescaledb') THEN
        PERFORM create_hypertable(
        ...
        RAISE NOTICE 'Created hypertable for processed_states';
    ELSE
        RAISE NOTICE 'TimescaleDB not available - using regular table for processed_states';
    END IF;
END $$;
